...
task_params.add_argument('--target_classes', type=int, default=4, required=False, help='number of classes')
task_params.add_argument('--init_classes', type=int, default=2, required=False, help='number of classes for the first task')
...
def run(args, verbose=True):
    ...
    num_training_samples, (train_datasets, test_datasets), config, classes_per_task = get_malware_multitask_experiment(
    ...
    #num_training_samples = 303331
    args.iters = 2 #int(np.ceil(num_training_samples/args.batch) * args.epoch)
    args.g_iters = args.iters #if args.g_iters is None else args.g_iters
